...
BASE_DIR <- "E:/Dropbox/@Manuscript 원고, 연구 topic/^mIO drug treatment_DSS TERM 조직재생학회지/!data deposit 데이터 논문/data 1/AIVIA"

OUT_DIR  <- "C:/iOrganoAssay"

IMG_DIR <- file.path(OUT_DIR, "microscopy")
SEG_DIR <- file.path(OUT_DIR, "segmentation")
...
for (folder in day_folders) {
  ...
  for (i in seq_along(img_files)) {
    ...
    new_name <- paste(mice, passage, day, well, sep = "_")
...
write.xlsx(metadata, file.path(OUT_DIR, "metadata.xlsx"))
